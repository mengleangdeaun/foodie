...
class BranchController extends Controller
{
    ...
    public function update(Request $request, Branch $branch)
    {
        ...
        $request->merge([
            ...
            'is_about_visible' => filter_var($request->is_about_visible, FILTER_VALIDATE_BOOLEAN) ? 1 : 0,
            'is_tos_visible' => filter_var($request->is_tos_visible, FILTER_VALIDATE_BOOLEAN) ? 1 : 0,
            'is_privacy_visible' => filter_var($request->is_privacy_visible, FILTER_VALIDATE_BOOLEAN) ? 1 : 0,
        ]);
        ...
        $data = $request->validate([
            ...
            'tax_rate' => 'nullable|numeric|min:0|max:100',
            'is_about_visible' => 'required|in:0,1',
            ...
            'is_tos_visible' => 'required|in:0,1',
            'is_privacy_visible' => 'required|in:0,1',
        ]);
        ...
        if ($request->hasFile('qr_payment_file')) {
            if ($branch->qr_payment_path)
                Storage::disk('public')->delete($branch->qr_payment_path);
    ...
    public function destroy($id)
    {
        ...
        if ($branch->qr_payment_path)
            \Storage::disk('public')->delete($branch->qr_payment_path);
        if ($branch->qr_path)
            \Storage::disk('public')->delete($branch->qr_path);
